...
<div class="page-wrap">
  ...
  <div class="group-1">
    ...
    <!-- ═══ 기존 설비 작화 (변경 없음) ═══ -->
      ...
      <img class="bcf-12-fire-1" src="<%= ctx %>/img/fireggg.gif" />
...
(function() {
  ...
  Object.keys(wordElMap).forEach(function(tag) {
    demoWord[tag] = { val: 600 + Math.random() * 600, next: Date.now() + Math.random() * STEP_MS };
  });
  Object.keys(tpElMap).forEach(function(tag) {
    var isCp = (tag === 'bcf12_d1081');
    demoTp[tag] = {
  ...
  function stepDemo(state, isCp, now) {
    var step, min, max;
    if (isCp) { step = (Math.random() - 0.5) * 0.1;  min = 0.1;  max = 0.9; }
    else       { step = (Math.random() - 0.5) * 60;   min = 600;  max = 1200; }
    state.val  = Math.min(max, Math.max(min, state.val + step));
    state.next = now + STEP_MS * (0.9 + Math.random() * 0.2);
  }
  ...
  function demoTick() {
    ...
    Object.keys(wordElMap).forEach(function(tag) {
      if (now >= demoWord[tag].next) stepDemo(demoWord[tag], false, now);
      data[tag] = Math.round(demoWord[tag].val);
    });
    Object.keys(tpElMap).forEach(function(tag) {
      var s = demoTp[tag];
      if (now >= s.next) stepDemo(s, s.isCp, now);
      data[tag] = s.isCp ? Math.round(s.val * 1000) : Math.round(s.val);
    });
