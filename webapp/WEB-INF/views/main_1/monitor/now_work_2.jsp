    ...
    <table id="workTable">
      ...
      <tbody>
        ...
        <tr class="sep" data-equip-cd="dongwoo_11" data-row-sort="1">
          ...
          <td class="data" data-col="prod"></td>
          <td class="data" data-col="lot"></td>
          <td class="st" data-tags="bcf11_164,bcf11_165"></td>
...
<script>
  ...
  /* PLC 공정현황 상태 (0=기본 / 1=연파랑) */
  var plcBusy = false;
  var plcTags = (function() {
    var set = {};
    document.querySelectorAll('td.st[data-tags]').forEach(function(td) {
      td.getAttribute('data-tags').split(',').forEach(function(t) { set[t.trim()] = true; });
    });
    return Object.keys(set);
  })();

  function fetchPlcStatus() {
    if (plcBusy || !plcTags.length) return;
    plcBusy = true;
    fetch(ROOT + '/monitor/main-data', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify(plcTags)
    })
    .then(function(r) { return r.ok ? r.json() : Promise.reject(r.status); })
    .then(function(data) {
      document.querySelectorAll('td.st[data-tags]').forEach(function(td) {
        var tags  = td.getAttribute('data-tags').split(',');
        var anyOn = tags.some(function(t) { return data[t.trim()] == 1; });
        td.classList.toggle('active', anyOn);
      });
    })
    .catch(function(e) { console.warn('[NowWork2] PLC fetch 실패:', e); })
