 ...
 *
 * 브라우저(AJAX 폴링) → 이 컨트롤러 → C# API(localhost:5050) → PLC 장비
 *
...
public class NewMonitoringController {
    ...
    private static RestTemplate createRestTemplate() {
        ...
        f.setConnectTimeout(1000);  // C# API 연결 타임아웃 1초 (localhost)
        f.setReadTimeout(5000);     // PLC 읽기 응답 타임아웃 5초
    ...
    /**
     * DOM 수집 태그 목록을 받아 설비별 병렬 읽기 후 태그→값 맵 반환.
     *
     * 동작 순서:
     *   1. 태그 파싱 → PLC 번호, 주소, 워드/비트 구분
     *   2. PLC별 min~max 범위로 묶어 C# API 1회 호출 (설비별 병렬)
     *   3. 모든 설비 완료 후 태그명으로 조합 반환
     *
     * 설비별 병렬 실행으로 한 설비의 느린 응답이 다른 설비에 영향을 주지 않음.
     * 타임아웃(5초) 초과 설비는 해당 설비 태그만 null 처리, 나머지는 정상 반환.
     */
    ...
    public Map<String, Object> mainData(@RequestBody List<String> tags) {
        ...
        // 설비별 병렬 읽기를 위해 ConcurrentHashMap 사용
        Map<Integer, Map<Integer, Object>> wordCache = new ConcurrentHashMap<>();
        Map<Integer, Map<Integer, Object>> bitCache  = new ConcurrentHashMap<>();
        ...
        wordAddrs.forEach((plcNum, addrs) ->
            futures.add(CompletableFuture.runAsync(() -> {
                ...
                for (TreeSet<Integer> group : Arrays.asList(lowGroup, highGroup)) {
                    ...
                    // 소주소(< 40000): FC03 raw read / 대주소(≥ 40000): 표준 Modbus 4x 주소
                ...
                if (!merged.isEmpty()) wordCache.put(plcNum, merged);
            }))
        );

        // 비트(신호) 읽기 — 설비별 병렬 (MODBUS_TCP 전용)
